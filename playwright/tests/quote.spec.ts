 ...
 *
 * In addition to the deterministic suite below, an agentic block loads
 * AI-generated edge case scenarios from `mcp/generated-scenarios.json`
 * via `getAgentScenarios('/quote')`.
 */
  ...
  test.describe('agentic scenarios', () => {
    ...
    if (scenarios.length === 0) {
      test('no agentic scenarios available (skipped)', async () => {
        console.log('No /quote agentic scenarios found — run `npm run scenarios` to generate them.');
      });
    }
    ...
    for (const s of scenarios) {
      test(s.name, async () => {
        const { response, data } = await client.getQuote(
          s.params as Parameters<LiFiApiClient['getQuote']>[0]
        );
